...
parser.add_argument('--train_pct', help='Ratio of images to go to train folder (example: ".7")',
                    default=.7)
parser.add_argument('--val_pct', help='Ratio of images to go to validation folder (example: ".2")',
                    default=.2)
parser.add_argument('--test_pct', help='Ratio of images to go to test folder (example: ".1")',
                    default=.1)
args = parser.parse_args()

data_path = args.datapath
train_percent = float(args.train_pct)
val_percent = float(args.val_pct)
...
total_pct = train_percent + val_percent + test_percent
if not (0.99 <= total_pct <= 1.01):
   print(f'train_pct + val_pct + test_pct must equal 1.0, got {total_pct:.2f}')
   sys.exit(0)
...
test_num  = file_num - train_num - val_num  # remainder goes to test
